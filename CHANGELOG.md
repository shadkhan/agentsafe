...
- Privacy documentation now describes the `chrome.storage.session` per-tab scan cache. The previous text stated page content was never stored, but the cache holds the sanitized export, including extracted main content. It is memory-backed and cleared when Chrome closes. Corrected in `PRIVACY.md`, `README.md`, and `ARCHITECTURE.md`. The policy is now a single file at the repository root; the previous split between a root summary and a fuller copy under `docs/` was itself a source of drift.
